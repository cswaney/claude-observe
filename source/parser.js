...
/**
 * Parse logs from a session directory
 ...
 */
export function parseSession(sessionDir, sessionId = null) {
	...
	// Parse main logs
	const logs = parseLogFile(path.join(sessionDir, mainFile));

	// Get session ID from the main file name (without .jsonl extension)
	const currentSessionId = path.basename(mainFile, '.jsonl');

	// Parse agent files that belong to this session
	const agentFiles = files.filter(f => f.startsWith('agent-') && f.endsWith('.jsonl'));
	for (const agentFile of agentFiles) {
		const agentPath = path.join(sessionDir, agentFile);
