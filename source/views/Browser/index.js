...
export default function Browser({
	...
	return (
		<Box flexDirection="column" width={width}>
					...
					{sessions.length === 0 ? (
						...
						<Box flexDirection="column">
							<Box>
								...
								<Box width={44} justifyContent="flex-start">
									<Text dimColor bold>
										Session
									</Text>
								</Box>
							...
							{hasItemsAbove && (
								<Box justifyContent="center">
									<Text dimColor>
										... {rowsAbove} {rowsAbove === 1 ? 'session' : 'sessions'}{' '}
										above
									</Text>
								</Box>
							)}
							...
							{visibleSessions.map((item, relativeIndex) => {
								...
								const selectedProject =
									sessions[selectedIndex]?.project ||
									sessions[selectedIndex]?.projectName;
								const itemProject = item.project || item.projectName;
								const isSameProject = itemProject === selectedProject;
								const created = item.birthtime
									? new Date(item.birthtime).toLocaleDateString()
									: 'N/A';
								const modified = item.mtime
									...
									<Box key={`session-${item.path}`}>
										...
										<Box width={44} justifyContent="flex-start" marginRight={3}>
											<Text
												...
												bold={isSelected}
												dimColor={!isSameProject || !isSelected}
												color={textColor}
											>
							...
							{hasItemsBelow && (
								<Box justifyContent="center">
									<Text dimColor>
										... {rowsBelow} {rowsBelow === 1 ? 'session' : 'sessions'}{' '}
										below
									</Text>
								</Box>
							)}
			...
			<Box justifyContent="center">
				<Text dimColor>
					↑/↓: Navigate sessions | →/Enter: Open session | /: Filter | u/d/t/b:
					Jump
				</Text>
			</Box>
		</Box>
