...
function TokenHistogramByType() {
	...
	return (
		<Box flexDirection="column" padding={1}>
			<Text bold color="white">Token Usage by Type</Text>
				...
				return (
					<Box key={name} flexDirection="column" marginBottom={1}>
						<Text bold color={color}>
							{label}: {data.totalTokens.toLocaleString()} tokens ({percentage}%) - {data.count} logs
						</Text>
						{data.x.length > 0 ? (
							<Histogram
								x={data.x}
			...
			<Text> </Text>
			<Text dimColor>Each histogram shows when that activity type occurred during the session.</Text>
		</Box>
	);
}
